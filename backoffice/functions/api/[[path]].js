/**
 * Cloudflare Pages Function — Same-origin API proxy for the backoffice app.
 *
 * Why this exists
 * ---------------
 * The browser lives on `backoffice.fufutcoffee.com` and the API lives on
 ...
 *      them so the cookie is stored on `backoffice.fufutcoffee.com`
 *      (first-party) instead of the Worker origin (third-party). Without
 *      this rewrite, the session would still never persist in the browser.
...
function rewriteSetCookie(cookie) {
  ...
  // (backoffice.fufutcoffee.com) instead of the Worker origin.
  ...
  // paths on the backoffice origin.
...
function buildOutgoingHeaders(request) {
  ...
  // Help the Worker recognise the original backoffice origin even when
  // the function itself sends a server-to-server fetch (no browser Origin).
  if (!headers.has('origin')) {
    headers.set('origin', 'https://backoffice.fufutcoffee.com');
  }
  if (!headers.has('referer')) {
    headers.set('referer', 'https://backoffice.fufutcoffee.com/');
  }
  return headers;
}
