/**
 * Cloudflare Pages Function — Same-origin API proxy for the POS app.
 *
 * Why this exists
 * ---------------
 * The browser lives on `pos.fufutcoffee.com` and the API lives on
 ...
 *      them so the cookie is stored on `pos.fufutcoffee.com` (first-
...
function rewriteSetCookie(cookie) {
  ...
  // (pos.fufutcoffee.com) instead of the Worker origin.
...
function buildOutgoingHeaders(request) {
  ...
  if (!headers.has('origin')) {
    headers.set('origin', 'https://pos.fufutcoffee.com');
  }
  if (!headers.has('referer')) {
    headers.set('referer', 'https://pos.fufutcoffee.com/');
  }
  return headers;
}
