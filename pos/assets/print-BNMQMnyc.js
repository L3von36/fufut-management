...
<body>
  ...
  ${n}
</body></html>`),s.document.close(),s.focus();let c=()=>{try{s.print(),s.close()}catch{}};return s.document.readyState===`complete`?setTimeout(c,60):s.onload=c,!0}function s(e=new Date){return e.toLocaleString()}function c(e,n=[]){let r=String(e.order_type||e.type||`dine-in`).replace(`-`,` `).toUpperCase(),i=e.tableNum||e.table_number||e.table_id,a=[`<div style="text-align:center;margin-bottom:6px"><span class="badge-type">${t(r)}</span></div>`,`<h1 style="font-size:1.6em">#${t(String(e.id||``).slice(-4))}</h1>`,i?`<div class="sub">Table ${t(i)}</div>`:``,e.customer?`<div class="sub">${t(e.customer)}</div>`:``,e.customer_phone?`<div class="sub">${t(e.customer_phone)}</div>`:``,`<hr>`,n.length?n.map(e=>{let n=e.modifiers&&e.modifiers!==`[]`?l(e.modifiers):``;return`<div class="ticket-item">${t(e.qty)} × ${t(e.name)}</div>`+(n?`<div class="ticket-mods">${t(n)}</div>`:``)+(e.notes?`<div class="ticket-mods">! ${t(e.notes)}</div>`:``)}).join(``):`<div class="ticket-item">${t(e.items||``)}</div>`,`<hr>`,e.notes?`<div class="ticket-item" style="font-size:1.2em">** ${t(e.notes)} **</div>`:``,`<div class="foot">${t(s())}</div>`].join(``);return o({title:`Kitchen ticket ${e.id}`,body:a,paper:`receipt`,subtitle:`KITCHEN`})}function l(e){let t=e;if(typeof t==`string`)try{t=JSON.parse(t)}catch{return t}return Array.isArray(t)?t.map(e=>typeof e==`string`?e:e&&e.name).filter(Boolean).join(`, `):``}function u({title:e,subtitle:r=``,summary:a=[],headers:c,rows:l,footer:u=``}){return o({title:e,body:[`<div class="meta"><strong>${t(e)}</strong>${r?` — `+t(r):``}</div>`,a.length?a.map(([e,t])=>n(e,t)).join(``)+`<hr>`:``,c&&l?i(c,l):``,u?`<div class="foot">${t(u)}</div>`:``,`<div class="foot">Printed ${t(s())}</div>`].join(``),paper:`a4`,subtitle:r})}export{u as n,c as t};